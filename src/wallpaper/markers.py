...
def apply_sunglint_marker_if_needed(
    *,
    wallpaper_path: Path,
    pic_side: int,
    observation_time: struct_time,
    ...
    """按 UTC 观测时间计算葵花视角海面耀斑点并标注；投影失败只记日志。"""
    lat, lon = sunglint_latlon(observation_time)
    logging.info("Sunglint point: lat=%.3f lon=%.3f", lat, lon)
    draw_typhoon_marker_at(
        wallpaper_path=wallpaper_path,
        pic_side=pic_side,
        lat=lat,
        ...
        margin_top_percent=margin_top_percent,
        margin_bottom_percent=margin_bottom_percent,
        label=_SUNGLINT_MARKER_LABEL,
        color=_SUNGLINT_MARKER_COLOR,
        style="sun",
    )
